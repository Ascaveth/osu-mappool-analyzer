...
The within-stage counterpart to `ProgressionAnalyzer`'s across-stage check, and the first Tournament Analyzer to consume real Star Rating rather than a proxy metric — see [docs/17-osu-api-integration.md](17-osu-api-integration.md) for how that data is fetched and stored. Where `ProgressionAnalyzer` asks "does average OD trend upward stage-to-stage," `DifficultySpreadAnalyzer` asks "within one stage, is the slot ordering itself a fair progression" — a different axis entirely, so it is a separate analyzer rather than an extension of `ProgressionAnalyzer`.

Star Rating is not read from `domain.Beatmap.StarRating` (which the import pipeline never populates — see [docs/08-beatmap-import-pipeline.md](08-beatmap-import-pipeline.md#what-is-explicitly-deferred)). It's read through an injected `StarRatingLookup` interface (`Find`, `FindAllForBeatmap`) satisfied as-is by `storage.StarRatingRepository`, keeping the analyzer itself pure and network-free like every other analyzer in this package — the actual network call happened earlier, at import-time enrichment.

**FreeMod handling.** A "FM" slot isn't skipped for lack of a single fixed mod; its difficulty is represented as a `[min, max]` range across `modmap.FreeModCandidates` (NoMod, HardRock, Easy — DoubleTime is deliberately excluded as not a legal FreeMod pick under this project's tournament convention). A slot is flagged only when its *entire* range falls outside the acceptable comparison, so an FM slot is never penalized just because one edge of its range looks anomalous. A slot in a category with no fixed-mod mapping and no FreeMod convention (e.g. "TB") is excluded from the sequence entirely (`skipped_slots_no_fixed_mod`), as is any slot whose Star Rating simply hasn't been fetched yet (`skipped_slots_no_sr_data`) — both are reported as metrics, not findings, since missing data is not itself a tournament-design problem.

Four findings, all Warning severity except the first (Info), each with its own named constant so the heuristic is revisitable rather than buried in logic:

- **Gap** — the distance between two consecutive slots' closer range edges (`edgeDistance`; `0` when ranges overlap) exceeds `gapMultiplier` (2.0) times the stage's own median consecutive gap (`medianPositive` over nonzero deltas). Self-referential to the stage, mirroring `ProgressionAnalyzer`'s spike heuristic, since no cross-tournament "expected gap" norm exists yet.
- **Spike** — a slot's range deviates from the mean of its immediate neighbors by more than `spikeDeviationThreshold` (1.0 star) — a fixed absolute threshold (not stage-relative like Gap) because a single-slot outlier is legible as "off" on Star Rating's own scale regardless of the stage's typical pacing.
- **Too tight** — a stage with at least `minSlotsForSpreadJudgment` (3, matching `SkillCoverageAnalyzer`'s `minSlotsForCoverageJudgment` convention to avoid judging small stages) usable slots but an overall max-min spread below `minExpectedSpread` (0.3 stars) — every map testing roughly the same difficulty rather than a progression. Severity Info, not Warning: a tight-but-intentional stage (e.g. a Qualifiers pool designed to be uniformly accessible) is plausible, unlike a Gap or Spike, which are never intentional.
- **Too wide** — overall spread exceeds `wideSpreadMultiplier` (3.0) times the stage's own median consecutive gap, catching a spread dominated by one or two large jumps rather than gradual progression even when no single gap crossed the Gap threshold on its own.
- **Deviation from projected target** — the mean usable Star Rating across the stage differs from the stage's projected target by more than `projectedDeviationThreshold` (0.5 stars). The target is the stage's explicit `ProjectedStarRating` if a tournament organizer set one, otherwise the analyzer computes its own NM1 fallback (first "NM"-category slot at position 1) directly from `StarRatingLookup` — deliberately *not* by calling the `EffectiveProjectedStarRating` helper (`tournament/projected_rating.go`, used by `api/dto.go` for API display), because that helper's NM1 fallback reads `domain.Beatmap.StarRating`, which is always `0` in this codebase; calling it here would manufacture a false deviation finding on every stage with no explicit projected rating.

Reports `filled_slots`, `usable_slots`, `skipped_slots_no_fixed_mod`, `skipped_slots_no_sr_data`, `fm_slots_ranged`, `sr_range`, `sr_median_delta`, `gap_count`, `spike_count`, and (when a target is resolvable) `projected_star_rating` and `deviation_from_projected`. `Score` is `1.0 - (gap_count + spike_count) / (usable_slots - 1)`, `nil` when fewer than 2 usable slots exist to compare.
